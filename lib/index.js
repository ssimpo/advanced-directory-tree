...
function _setDefaultOptions(options) {
	...
	return _copyProperties({
		...
		'ignoreList': ignoreListToRegX(options.ignoreList),
		'fileExtensions': options.fileExtensions || []
	}, options);
}

function ignoreListToRegX(ignoreList){
	return (ignoreList || []).map(function(ignoreGlob){
		return globX(ignoreGlob);
	});
}
...
function _shouldIgnoreThisPath(filePath, ignoreList) {
	if (ignoreList.length) {
		var ignore = false;
		ignoreList.forEach(function (test) {
...
function directoryTree(basepath, options) {
	...
	makeArray(basepath).reverse().forEach(function(basepath){
		var treeSource = _directoryTree(basepath, options);
		if(tree){
