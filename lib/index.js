...
function _directoryTreeAsync(basepath, options, callback) {
	var treeCreationPromise = stat(basepath).then(function (stats) {
		...
		if (stats.isFile()) {
			return (
				(_isFileExtensionMatch(basepath, options.fileExtensions)) ?
					_getFileItem(basepath, stats) :
					null
			);
		} else {
			return _getChildren(
				readdir, basepath, options
			).then(function (children) {
				var item = _getDirectoryItem(basepath, children);
				return (
					(options.hideEmptyDirectories && item.children.length === 0) ?
						null :
						item
				);
			});
...
function directoryTree(basepath, options) {
	...
	if (stats.isFile()) {
		...
		var item = _getDirectoryItem(
			basepath,
			_getChildren(fs.readdirSync, basepath, options)
		);
		return (
			(options.hideEmptyDirectories && item.children.length === 0) ?
				null :
...
function directoryTreeAsync(basepath, options, callback) {
	...
	_directoryTreeAsync(basepath, options, callback);
}
