...
function directoryTree(basepath, options) {
  ...
  if (stats.isFile()) {
    ...
    var item = getDirectoryItem(basepath, fs.readdirSync(basepath).map(function (child) {
...
function directoryTreeAsync (basepath, options, callback) {
  ...
  return stat(basepath).then(function(stats){
    ...
    if (stats.isFile()) {
      ...
      return readdir(basepath).map(function (child) {
        return directoryTreeAsync(path.join(basepath, child), options);
      }).filter(function (e) {
        return e !== null;
